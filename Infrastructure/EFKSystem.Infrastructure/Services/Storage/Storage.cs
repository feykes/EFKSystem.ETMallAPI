using EFKSystem.Infrastructure.Operations;
using System;
...
namespace EFKSystem.Infrastructure.Services.Storage
{
        ...
        protected async Task<string> FileRenameAsync(string pathOrContainer, string fileName, HasFile hasFileMethod, bool first = true)
        {
            string newFileName = await Task.Run<string>(async () =>
            {
                ...
                if (first)
                {
                    ...
                    newFileName = $"{NameOperation.CharacterRegulatory(oldName)}{extension}"; //yeni ismin sonuna uzantısını ekledik
                }
                else
                {
